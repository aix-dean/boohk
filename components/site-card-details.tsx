...
const GoogleMap = React.memo(({ location, className, lat, lng }: { location: string; className?: string; lat?: number; lng?: number }) => {
  ...
  useEffect(() => {
    if (mapLoaded && mapInstanceRef.current && mapCenter) {
      const handleMapClick = () => {
        const url = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${mapCenter.lat()},${mapCenter.lng()}`)}`
  ...
  if (mapError) {
    return (
      <div onClick={handleErrorClick} className={`bg-gray-100 rounded-lg flex items-center justify-center cursor-pointer ${className}`}>
          ...
          {(product?.type?.toLowerCase() === "rental" ? product.specs_rental?.location : product.light?.location) && (
            <div className="flex-1 aspect-square overflow-hidden">
              <GoogleMap
                ...
                lng={product?.specs_rental?.geopoint?.longitude}
