  ...
  return (
    <div className="w-full max-w-full overflow-hidden">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full max-w-full">
        <div className="bg-indigo-600 text-white rounded-lg px-3 py-3 sm:px-6 sm:py-[17px] min-w-0 max-w-full">
          <div className="text-xs mb-1 sm:mb-2 truncate">Total Transactions</div>
          <div className="text-sm sm:text-lg sm:text-xl font-semibold truncate">{totalTransactions}</div>
        </div>
        <div className="bg-white rounded-lg px-3 py-3 sm:px-6 sm:py-[17px] border border-gray-200 min-w-0 max-w-full">
          <div className="text-xs text-gray-600 mb-1 sm:mb-2 truncate">Total Retail Revenue</div>
          <div className="text-sm sm:text-lg sm:text-xl font-semibold text-gray-900 truncate">
            ₱{totalRetailRevenue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </div>
        </div>
        <div className="bg-white rounded-lg px-3 py-3 sm:px-6 sm:py-[17px] border border-gray-200 min-w-0 max-w-full">
          <div className="text-xs text-gray-600 mb-1 sm:mb-2 truncate">Fees and Commissions</div>
          <div className="text-sm sm:text-lg sm:text-xl font-semibold text-gray-900 truncate">
            ₱{totalFees.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </div>
        </div>
        <div className="bg-white rounded-lg px-3 py-3 sm:px-6 sm:py-[17px] border border-gray-200 min-w-0 max-w-full">
          <div className="text-xs text-gray-600 mb-1 sm:mb-2 truncate">Net Sales</div>
          <div className="text-sm sm:text-lg sm:text-xl font-semibold text-gray-900 truncate">
