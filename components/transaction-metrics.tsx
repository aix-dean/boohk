...
export function TransactionMetrics({ transactions }: TransactionMetricsProps) {
  ...
  const totalFees = transactions.reduce((sum, t) => sum + (t.fees?.totalFee || 0), 0)
  const netSales = transactions.reduce((sum, t) => sum + (t.fees?.netAmount || 0), 0)
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      ...
      <div className="bg-white rounded-lg px-6 py-[17px] border border-gray-200">
        <div className="text-xs text-gray-600 mb-2">Fees and Commissions / Payout</div>
        <div className="text-xl font-semibold text-gray-900">
