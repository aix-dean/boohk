...
export async function GET(
  ...
  try {
    console.log('[DEBUG] Starting GET request for conversationId:', params.conversationId)
    ...
    console.log('[DEBUG] Fetching conversation document:', conversationId)
          ...
          status: {
            delivered: Object.fromEntries(
              Object.entries(data.status?.delivered || {}).map(([k, v]) => {
                ...
                return [k, (v as Timestamp)?.toDate ? (v as Timestamp).toDate() : new Date()]
              })
            ),
